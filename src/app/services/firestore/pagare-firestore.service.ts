  ...
  async pdfDownload(userData: any, documentData: any){
    ...
    if(this.plt.is('capacitor')){
      this.pdfOjb.getBuffer((buffer: Uint8Array) => {
        ...
        this.file.writeFile(this.file.dataDirectory, `${documentData.pagareName}.pdf`, blob, { replace: true }).then(fileEntry =>{
          this.fileOpener.open(this.file.dataDirectory + `${documentData.pagareName}.pdf`, 'application/pdf');
        });
      });
      return true;
    }else{
